...
const logFile = path.resolve(__dirname, 'seed.log');
...
log(`KEY: ${SUPABASE_KEY.substring(0, 10)}...`);
...
async function seed() {
    log('🌱 Seeding data for Ranking Agent test...');
    ...
    try {
        ...
        if (jobError) {
            log(`❌ Error creating job: ${JSON.stringify(jobError)}`);
            return;
        }
        ...
        if (appError) {
            log(`❌ Error creating application: ${JSON.stringify(appError)}`);
            return;
        }
        log(`✅ Application created: ${app.id}`);
        log(`\n🚀 RUN THIS COMMAND: npx tsx scripts/ranking-agent.ts --job-id=${job.id}`);
    } catch (e: any) {
        log(`❌ Global Catch: ${e.message}`);
    }
}
